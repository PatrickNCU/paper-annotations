...
2. **確認筆記要放哪**。使用者沒指定就用預設（產物放進論文套件內，整包可攜），
   但要**明講這件事**，不要默默決定。他想放別處就用 `probe.py --out <路徑>`。
...
4. **首次建置**：`build_annotated.py` 再 `build_html.py`，把複習頁的路徑給他。

5. **告訴他接下來怎麼用**：直接問論文問題就好，疑問會自動累積成卡片；不必記任何指令。

完成條件：使用者知道自己在第幾層、筆記會產生在哪、複習頁在哪個檔案，而且知道
下一步只要開口問問題。
