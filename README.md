...
複習頁是**一個離線 HTML**，加 `--embed-assets` 連圖片都內嵌，寄出去對方點兩下就能讀：
不用裝 Python、不用開 server、手機平板都行。他可以照樣畫記、寫註解、篩選、搜尋，
畫記存在他自己的瀏覽器；想回饋給你就按「複製畫記」把內容傳回來。
...
- 要寄給別人就加 `--embed-assets`，圖片一併內嵌成單一檔案（見上面〈寄給別人〉）
