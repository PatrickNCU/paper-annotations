...
- 讀的時候可以自己用螢光筆畫記，畫記存在瀏覽器本機，隨時複製回對話交給 agent
...
- **側邊欄滑出式**：滑鼠移到左上角的按鈕就滑出來，蓋在論文上方——**論文一格都不會位移**。
  ...
  靠這裡進入）。要它留著就點一下把手或按「釘住」，觸控裝置沒有 hover，也是用點的
- **右邊有提問草稿區**：讀完整段再一次把問題寫下來，不必讀到一半打斷去問。打開時
  論文會讓開，不會被蓋住；寫完按「複製」貼回對話。內容存在瀏覽器本機，重開還在。
  它只是草稿——不會送出，也不會自己變成卡片
- **螢光筆**：選取正文就會浮出調色盤，三個顏色。畫完調色盤會留著，可以馬上改色或清掉；
  移開滑鼠、捲動頁面或按 Esc 就收起來。點既有的畫記也能叫回它。畫記**存在這台
  瀏覽器裡**（跟草稿區一樣），不進卡片、不進 git——複習頁是離線單檔，沒有後端可以寫檔。
  要留下來就按側欄的「複製畫記」，貼回對話交給 agent 處理。重建頁面後畫記會自動重新
  定位（用引文加前後文比對）；真的對不上的**不會被丟掉**，側欄會報數字，複製時照樣列出
