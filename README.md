...
卡片是唯一真實來源，手改是預期用法。平常是 agent 直接寫這些檔案（沒有產生器腳本，
...
（`notes/marks/` 是螢光筆畫記，格式跟卡片同一套 frontmatter，內文就是註解。）

所以**只有 `notes/cards/` 值得改**，而且改了**一定會**反映到複習頁——重建之後。
...
- **螢光筆＋註解**：選取正文就會浮出調色盤，四個顏色，✎ 可以在那條畫記上寫一段話
  （帶註解的畫記會多一條虛底線，點了就展開）。畫完調色盤會留著，可以馬上改色或清掉；
  移開滑鼠、捲動頁面或按 Esc 就收起來。畫記先**存在這台瀏覽器裡**——複習頁是離線單檔，
  沒有後端可以寫檔。按側欄的「複製畫記」貼回對話，agent 用 `import_marks.py` 寫成
  `notes/marks/`（重跑安全，不會寫出重複的）；落檔後就跟卡片一樣進 git、重建也還在，
  側欄會告訴你還有幾條沒落檔。**滑鼠移到有註解的畫記上就浮出註解**，點下去看完整內容。
  對不上原文的畫記**不會被丟掉**，匯入與 build 都會逐條報出來
...
複習頁本身是**離線單檔**，點兩下就能讀，什麼都不用裝——這是預設，也是寄給別人時的樣子。
但瀏覽器裡的頁面沒辦法寫檔或執行程式（這是瀏覽器的安全模型，不是這裡少做了什麼），
所以畫記只能先留在瀏覽器，再由你複製出來交給 agent。
...
或直接 `/paper-annotations:serve`，它會在背景跑起來、把網址給你，並在論文資料夾裡放一個
**點兩下就能開**的啟動器（Windows `開啟複習頁.cmd`，其他系統 `.command`）。之後不必開
對話也能自己開複習頁；那個視窗就是 server，關掉它就停。

複習頁上會多一顆「💾 存檔」：按下去就把畫記寫進 `notes/marks/`、重建頁面、重新整理。
沒開 server 的時候那顆按鈕根本不會出現（它靠探測後端決定），其餘功能完全一樣。

**開著 server 時，已落檔的畫記在頁面上就能直接改**：點它一樣浮出調色盤，換顏色、✎ 改
註解、✕ 刪掉——都是直接覆寫或刪除 `notes/marks/` 裡那個檔，然後重建。沒有 server 時
它是唯讀的（點了顯示註解），要改就編輯檔案本身。

一個 server 對一篇論文：換論文就換參數，要同時開就換 `--port`。畫記只會寫進那一篇的
`notes/marks/`，而且請求裡指定的章節必須是那篇 `paper.yml` 已宣告的，跨論文寫入不可能。

**畫記不會跨著 `file://` 和 `http://` 走**：瀏覽器把這兩者當成不同來源，各有各的
localStorage。開了 server 之後就固定從 `http://127.0.0.1:…` 讀，先前直接開檔案時
畫、還沒落檔的畫記要先在原本那個分頁按「複製畫記」帶出來。

| | 直接開檔案 | 跑 serve.py |
|---|---|---|
| 讀論文、卡片、畫記、註解、匯出 | 全部一樣 | 全部一樣 |
| 畫記存到哪 | 瀏覽器，要複製給 agent | 一鍵寫進 `notes/marks/` |
| 改／刪已落檔的畫記 | 編輯 `notes/marks/` 的檔案 | 在頁面上直接改，覆寫該檔 |
| 清瀏覽器資料／換電腦 | 沒落檔的會不見 | 不受影響 |
| 寄給同學 | ✅ 一個檔就好 | 不需要 |

安全上做了什麼：**只綁 `127.0.0.1`**（同網段的人連不到，實測從本機 LAN 位址連是拒絕的）；
寫入只限 `notes/marks/`，檔名一律由 server 產生，請求裡的路徑必須是這篇論文已宣告的
章節之一，其餘一概不收；存檔要帶啟動時產生的 token，而且檢查 `Origin`——瀏覽器裡
別的網頁既讀不到 token（不送任何 CORS 標頭），也送不出那個自訂標頭。
