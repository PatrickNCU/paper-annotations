...
from . import (
    actions, checks, cli, graph, katex, library, links, minimd, notes,
...
def build(work_root: Path, embed: bool = False, to: str = "") -> int:
    ...
    checks_css = (ASSETS / "checks.css").read_text(encoding="utf-8")
    checks_js = (ASSETS / "checks.js").read_text(encoding="utf-8")

    # Where every checkpoint stands, replayed from its log. Only the state
    # travels: the question is already in the text, and the section's points
    # and the answer sentence are already elements on the page.
    check_rows, check_counts = checks.tally(notes_dir, checks.load_checks(notes_dir))
    ...
    # The way back to the shelf, as a path relative to this file, so it works
    # from a folder as well as from a server. Only when there really is one:
    # a paper with no registry, or one whose shelf has never been built, gets
    # no button rather than a button that 404s.
    shelf_href = ""
    if registry is not None:
        shelf_file = registry.parent / "library.html"
        if shelf_file.is_file():
            shelf_href = os.path.relpath(shelf_file, annotated).replace("\\", "/")
    ...
    body_parts, toc, used = [], [], {}
    missing = []
    # A section's own heading is the first one its file produces, which is what
    # a checkpoint's state gets pinned to in the contents.
    section_head = {}
    ...
    for rel in source_list:
        ...
        if len(toc) > first_heading:
            section_head[rel.stem] = toc[first_heading][2]
        elif toc:
            # A section that continues the previous one has no heading of its
            # own -- its state belongs on the heading it is sitting under, and
            # the contents then carries two marks on that one line.
            section_head[rel.stem] = toc[-1][2]
    ...
    for row in check_rows:
        row["anchor"] = section_head.get(Path(row["section"]).stem, "")
    check_json = json.dumps(
        {"checks": check_rows, "max": checks.MAX_TRIES}, ensure_ascii=False
    ).replace("</", "<\\/")
...
"""
    ...
    if check_rows:
        print(
            f"            段落檢查點 {len(check_rows)} 個：過了 {check_counts['pass']}、"
            f"跳過 {check_counts['skip']}、成了卡片 {check_counts['card']}、"
            f"還沒答 {check_counts['todo'] + check_counts['again']}"
        )
        print("                 作答要 serve.py 在跑，結果才寫得進 notes/checks/answers/")
