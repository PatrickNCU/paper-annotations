...
from . import anchors, cli, miniyaml, notes, workspace
...
def write_marks(paper_root: Path, notes_dir: Path, records):
    ...
    Shared with the server, so the save button and the command line cannot
    drift apart. A mark is identified by its file plus its quote, which is
    what makes a repeat run write nothing.
    """
    marks_dir = notes_dir / "marks"
    seen, used = set(), set()
    for mark in notes.load_marks(notes_dir):
        anchor = mark["meta"].get("anchor") or {}
        quote = anchor.get("quote") or {}
        seen.add((str(anchor.get("file") or ""),
                  anchors.normalize(str(quote.get("exact") or ""))))
        used.add(str(mark["meta"].get("id")))

    out = {"written": 0, "skipped": 0, "bad": [], "soft": [], "files": []}
    for rec in records:
        key = (rec["file"], anchors.normalize(rec["exact"]))
        if key in seen:
            out["skipped"] += 1
            continue
        if rec["color"] not in notes.VALID_COLOR:
            rec["color"] = "yellow"
        ...
        hits = anchors.count_quote(lines, anchors.normalize(rec["exact"]))
        if hits == 0:
...
def main(argv) -> int:
    ...
    args = cli.positionals(argv, value_flags={"--from"})
    work = Path(args[0] if args else ".").resolve()
    _, paper_root, notes_dir, _ = workspace.load_workspace(work)

    src = cli.flag(argv, "from")
    ...
    result = write_marks(paper_root, notes_dir, records)
