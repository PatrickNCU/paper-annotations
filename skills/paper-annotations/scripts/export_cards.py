...
def card_rows(work_root: Path, statuses, tags):
    ...
    rows = []
    for card in paperkit.load_cards(notes):
        meta = card["meta"]
        ...
        rows.append(
            {
                ...
                "quote": paperkit.quote_text(quote),
...
def main(argv):
    ...
    fmt = (flag(rest, "format", "anki") or "anki").lower()
    if fmt not in ("anki", "csv", "json"):
        raise SystemExit(f"不認得的格式 {fmt}，可用：anki、csv、json")

    statuses = [s.strip() for s in (flag(rest, "status", "") or "").split(",") if s.strip()]
    tags = [t.strip() for t in (flag(rest, "tag", "") or "").split(",") if t.strip()]
